...
col1.metric(
    f"Chiffre d'affaires prévisionnel à fin {datetime.datetime.now().strftime('%B %Y')}",
    f"{ca_auj: ,} €",
    f"{round(((ca_auj/ca_année_précédente-1)*100),2)}% par rapport à {datetime.datetime.now().replace(year=datetime.datetime.now().year - 1).strftime('%B %Y')}",
)
col2.metric(
    f"Marge prévisionnelle à fin {datetime.datetime.now().strftime('%B %Y')}",
    f"{marge_auj: ,} €",
    f"{round(((marge_auj/marge_année_précédente-1)*100),2)}% par rapport à {datetime.datetime.now().replace(year=datetime.datetime.now().year - 1).strftime('%B %Y')}",
)
col3.metric(
...
with col1:
    if periode == "Tout":
        ...
        data = data[
            (data["Mois"] <= datetime.datetime.now().strftime("%Y-%m"))
            & (
                data["Mois"]
                > datetime.datetime.now()
